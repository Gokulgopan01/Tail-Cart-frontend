...
#         self.root.after(1000, lambda: self.root.attributes('-topmost', False))  # Allow other windows to be on top after a short delay
...
#         self.selection_window.after(1000, lambda: self.root.attributes('-topmost', False))  # Allow other windows to be on top after a short delay
...
#         # Account Dropdown
#         ttk.Label(frame, text="Account:", font=label_font).pack(anchor="center", **padding)
...
#         #self.selection_window.iconify()  # Minimizes the main window    
...
class UserSelectionApp:
    def __init__(self, root):
        ...
        self.client_data = {}  # Stores client details
        self.create_login_frame()  # Load login UI

    def create_login_frame(self):
        self.login_frame = ttk.Frame(self.root, padding=20)
        self.login_frame.pack(fill="both", expand=True)

        ttk.Label(self.login_frame, text="ECESIS", font=("Arial", 14, "bold")).pack(pady=10)
        ttk.Label(self.login_frame, text="HYBRID CLIENT LOGIN", font=("Arial", 16, "bold")).pack(pady=20)
        ttk.Label(self.login_frame, text="Welcome back! Please login to your account to continue", font=("Arial", 10)).pack(pady=5)
        # Frame with Border for Email and Password
        input_frame = ttk.LabelFrame(self.login_frame , padding=10)
        input_frame.pack(pady=10, padx=10, fill="x")
        ...
        self.show_password_btn.pack(side="right", padx=(0, 5))
        # Buttons
        ttk.Button(self.login_frame, text="LOGIN", command=self.login, width=20).pack(pady=15)
        ttk.Button(self.login_frame, text="FORGET PASSWORD", command=self.forgot_password).pack(pady=5)

        self.welcome_label = ttk.Label(self.login_frame, text="", font=("Arial", 12, "bold"), foreground="green")
        self.welcome_label.pack(pady=5)
    ...
    #     """Simulates login and switches to client selection UI."""
    #     self.username = username  # Store username for later use
    #     if username:
    #         self.show_client_login(username)
    #     else:
    ...
    def show_client_login(self, username):
        ...
        # Create a new frame for client selection (centered)
        self.client_frame = ttk.Frame(self.root, padding=20)
        self.client_frame.place(relx=0.5, rely=0.5, anchor="center")  # Center frame
        ...
        ttk.Label(
            self.client_frame,
            text="Select Client Details",
            font=("Arial", 14, "bold")
        ).grid(row=1, column=0, columnspan=2, pady=(0, 15))

        label_font = ("Arial", 11)
        dropdown_width = 30  # Increased width for better visibility

        # Main Client Dropdown
        ttk.Label(self.client_frame, text="Main Client:", font=label_font).grid(row=2, column=0, sticky="w", padx=10, pady=5)
        self.main_client_var = tk.StringVar()
        self.main_client_dropdown = ttk.Combobox(self.client_frame, textvariable=self.main_client_var, width=dropdown_width)
        self.main_client_dropdown.grid(row=2, column=1, padx=10, pady=5, sticky="ew")
        self.main_client_dropdown.bind("<<ComboboxSelected>>", self.on_main_client_select)

        # Sub Client Dropdown
        ttk.Label(self.client_frame, text="Sub Client:", font=label_font).grid(row=3, column=0, sticky="w", padx=10, pady=5)
        self.sub_client_var = tk.StringVar()
        self.sub_client_dropdown = ttk.Combobox(self.client_frame, textvariable=self.sub_client_var, width=dropdown_width)
        self.sub_client_dropdown.grid(row=3, column=1, padx=10, pady=5, sticky="ew")
        self.sub_client_dropdown.bind("<<ComboboxSelected>>", self.on_sub_client_select)

        # Portal Dropdown
        ttk.Label(self.client_frame, text="Portal:", font=label_font).grid(row=4, column=0, sticky="w", padx=10, pady=5)
        self.portal_var = tk.StringVar()
        self.portal_dropdown = ttk.Combobox(self.client_frame, textvariable=self.portal_var, width=dropdown_width)
        self.portal_dropdown.grid(row=4, column=1, padx=10, pady=5, sticky="ew")
        self.portal_dropdown.bind("<<ComboboxSelected>>", self.on_portal_select)

        # Account Dropdown
        ttk.Label(self.client_frame, text="Account:", font=label_font).grid(row=5, column=0, sticky="w", padx=10, pady=5)
        self.account_var = tk.StringVar()
        self.account_dropdown = ttk.Combobox(self.client_frame, textvariable=self.account_var, width=dropdown_width)
        self.account_dropdown.grid(row=5, column=1, padx=10, pady=5, sticky="ew")

        # Buttons - Centered below dropdowns
        self.login_button = ttk.Button(self.client_frame, text="Login to Portal", command=self.confirm_selection)
        self.login_button.grid(row=6, column=0, columnspan=2, pady=15)

        self.logout_button = ttk.Button(self.client_frame, text="Logout", command=self.logout)
        self.logout_button.grid(row=7, column=0, columnspan=2, pady=5)  # Logout below Login
