...
#     account_dropdown = ttk.Combobox(selection_window, textvariable=account_var, values=[a["username"] for a in accounts])
#     account_dropdown.pack(pady=5)
    
#     tk.Button(selection_window, text="Submit", command=lambda: messagebox.showinfo("Selection", f"Main Client: {main_client_var.get()}\nSub Client: {sub_client_var.get()}\nPortal: {portal_var.get()}\nAccount: {account_var.get()}")).pack(pady=10)
