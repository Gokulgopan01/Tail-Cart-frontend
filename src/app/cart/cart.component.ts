...
export class CartComponent implements OnInit {
  ...
  loadCart(): void {
    this.loading = true;
    this.http.get<CartItem[]>(`https://bulbous-unaesthetical-albert.ngrok-free.dev/api/user/cart/?user_id=${this.userId}`)
      .subscribe({
        next: (res) => {
          this.cartItems = res.map(item => ({
            ...
            product_image: item.product_image?.startsWith('http')
              ? item.product_image
              : 'https://bulbous-unaesthetical-albert.ngrok-free.dev' + item.product_image
          }));
          this.calculateTotal();
          this.loading = false;
        },
  ...
  updateQuantity(item: CartItem, newQuantity: number): void {
    ...
    this.http.put(`https://bulbous-unaesthetical-albert.ngrok-free.dev/api/user/cart/`, {
      user_id: this.userId,
  ...
  removeItem(item: CartItem): void {
    Swal.fire({
      ...
      this.http.delete(`https://bulbous-unaesthetical-albert.ngrok-free.dev/api/user/cart/?cart_id=${item.cart_id}`)
        .subscribe({
          next: () => {
  ...
  checkout(): void {
    ...
    Swal.fire({
      ...
      this.loading = true;
      this.http.post(`https://bulbous-unaesthetical-albert.ngrok-free.dev/api/checkout/`, {
        user_id: this.userId,
