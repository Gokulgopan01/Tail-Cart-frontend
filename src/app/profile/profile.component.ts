...
export class ProfileComponent implements OnInit {
  ...
  deletePet(pet: any): void {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.http.delete(`http://127.0.0.1:8000/api/user/pets/${pet.pet_id}/`)
          .subscribe({
            next: () => {
  ...
  loadProfile(): void {
    this.isLoading = true;
    this.http.get<UserProfile>(`http://127.0.0.1:8000/api/user/profile/?user_id=${this.userId}`)
      .subscribe({
        next: (response) => {
  ...
  onSubmit(): void {
    ...
    const request$ = this.hasProfile
      ? this.http.patch<ProfileResponse>('http://127.0.0.1:8000/api/user/profile/', profileData)
      : this.http.post<ProfileResponse>('http://127.0.0.1:8000/api/user/profile/', profileData);
  ...
  savePet(): void {
    ...
    const request$ = this.editingPet
      ? this.http.put('http://127.0.0.1:8000/api/user/pets/', { ...payload, pet_id: this.currentPet.pet_id, user_id: this.userId })
      : this.http.post('http://127.0.0.1:8000/api/user/pets/', payload);
