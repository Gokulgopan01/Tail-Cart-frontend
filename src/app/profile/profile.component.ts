...
export class ProfileComponent implements OnInit {
  ...
  deletePet(pet: any): void {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.http.delete(`https://tailcart.duckdns.org/api/user/pets/${pet.pet_id}/`)
          .subscribe({
            next: () => {
  ...
  loadProfile(): void {
    this.isLoading = true;
    this.http.get<UserProfile>(`https://tailcart.duckdns.org/api/user/profile/?user_id=${this.userId}`)
      .subscribe({
        next: (response) => {
  ...
  onSubmit(): void {
    ...
    const request$ = this.hasProfile
      ? this.http.patch<ProfileResponse>('https://tailcart.duckdns.org/api/user/profile/', profileData)
      : this.http.post<ProfileResponse>('https://tailcart.duckdns.org/api/user/profile/', profileData);
  ...
  loadPets(): void {
    ...
    this.http.get<any[]>(`https://tailcart.duckdns.org/api/user/pets/?user_id=${this.userId}`)
      .subscribe({
        next: (response) => {
          this.pets = response;
  ...
  savePet(): void {
    ...
    const request$ = this.editingPet
      ? this.http.put('https://tailcart.duckdns.org/api/user/pets/', { ...payload, pet_id: this.currentPet.pet_id, user_id: this.userId })
      : this.http.post('https://tailcart.duckdns.org/api/user/pets/', payload);
